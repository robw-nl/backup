#!/bin/bash
#
# Robust backup script for Linux systems (tested on Manjaro and Arch).
#
# This script creates backups of a specified source directory to a backup 
# directory (on another mount (drive)), manages old backups based on a retention cycle, and logs
# errors and notifications.
#
# For daily execution, create an autostart entry or a symlink to the script in
# cron.daily: sudo ln -s /home/rob/Files/Scripts/backup.sh /etc/cron.daily/backup
#
# Author: Rob Wijhenke
# Initial Release: March 6 2020
# Current Version: v3.0 January 4 2026
#
# Version History:
...
# v3.0   January 4 2026            Some tweaks
# ~ end comments ~
...
# execute this command to restore everything installed during the time of the last backup
# sudo pacman -S --needed - < pkglist.txt
# pamac build $(cat pkglist_aur.txt)

generate_package_list() {
    log_and_notify "Generating installed package list..." false

    # 1. Official Repo Packages (Native)
    pacman -Qqe > "/home/rob/Files/Scripts/pkglist.txt"

    # 2. AUR / Foreign Packages (CachyOS/paru)
    paru -Qm > "/home/rob/Files/Scripts/pkglist_aur.txt"
}
...
log_and_notify() {
    ...
    # ALWAYS use notify-send for the subtle UI progress updates
    if command -v notify-send >/dev/null 2>&1; then
        notify-send "Backup" "${message}"
    else
        echo "${message}"
    fi

    # Log to file ONLY if it is an actual error
    if [[ "${isError}" = true ]] && [[ -n "${message}" ]]; then
        if [[ ! -f "${logFile}" ]]; then
            touch "${logFile}"
        fi
        printf "[%s] %s\n" "$(date +%T)" "${message}" >> "${logFile}"
    fi
}
...
execute_backup() {
    ...
    sudo rsync -a --progress --backup-dir="$backup_dir" --delete -b -s \
    --exclude-from "${SCRIPTS}/backupexclude.txt" \
    --include-from "${SCRIPTS}/backupinclude.txt" \
    "${SOURCE}" "${BACKUP}" 2>> "${BACKUP_LOG}"
    rsync_result=$?
...
delete_old_backups() {
    ...
    # --- Retention Logic Correction for 'find -mtime' ---
    # The RETENTION_CYCLE (N) in the config represents the TOTAL number of folders to keep.
    # The 'find -mtime +X' utility usually keeps X+2 folders.
    # To achieve N total folders, we calculate the threshold as N - 2.
    local FIND_THRESHOLD=$((RETENTION_CYCLE - 2))

    # Safety check: Ensure the threshold is not negative.
    if [[ $FIND_THRESHOLD -lt 0 ]]; then
        FIND_THRESHOLD=0
    fi
    # --- End Correction ---

    # FIX APPLIED: Added -maxdepth 1
    # This prevents 'find' from diving into backup subdirectories, which avoids:
    # 1. "Permission denied" errors on root-owned system files.
    # 2. "No such file" errors when the parent folder is deleted while processing.
    find "$OLD_BACKUP_DIR" -mindepth 1 -maxdepth 1 -type d -mtime +"$FIND_THRESHOLD" \
        -exec sh -c 'sudo rm -rf "$1" 2>/dev/null; test $? -eq 0' sh {} \;

    find_result=$?

    if [[ $find_result -eq 0 ]]; then
        log_and_notify "Old backups removed (Threshold: +$FIND_THRESHOLD days)" false
    else
...
handle_backup_logs() {
    if [[ -f "${BACKUP_LOG}" ]]; then
        if [[ -s "${BACKUP_LOG}" ]]; then
            # An error occurred: Use Kate to show you exactly what happened
            ${EDITOR_CMD} "${BACKUP_LOG}" &
            log_and_notify "Errors occurred during backup. Opening log." true
        else
            # No errors: Delete the empty log and stay quiet
            rm "${BACKUP_LOG}"
...
# for easy reinstall (see comments above)
generate_package_list
